...
Функция ПолучитьМассивПоказателейДляРасчета() Экспорт 
...
Функция ПолучитьНепериодичныеДанные(СтруктураПараметров) Экспорт 
	
	Дата1 = НачалоДня(СтруктураПараметров.Дата1);
	Дата2 = КонецДня(СтруктураПараметров.Дата2);
	СтандартОтчетности = СтруктураПараметров.СтандартОтчетности;
	...
	|	И ЗначенияПоказателей.Периодичность В (&ПериодичностьНет, &ПустаяПериодичность)
...
Функция ПолучитьПериодичныеДанные(СтруктураПараметров) Экспорт 	
	...
	СтандартОтчетности = СтруктураПараметров.СтандартОтчетности;
	КолвоПериодовДляОсмотраНазад = СтруктураПараметров.КолвоПериодовДляОсмотраНазад;
	МассивПоказателейДляРасчета = ?(СтруктураПараметров.Свойство("МассивПоказателейДляРасчета"),
									СтруктураПараметров.МассивПоказателейДляРасчета,
									ПолучитьМассивПоказателейДляРасчета());
	ОтборПоКонтрагенту = Неопределено;
	СтруктураПараметров.Свойство("ОтборПоКонтрагенту", ОтборПоКонтрагенту);
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	МАКСИМУМ(ВТ_КонтрагентыИПериоды.Период) КАК Период
	...
	|				ИЛИ ВТ_КонтрагентыИПериоды.Период = ВТ_МаксимальныйПериод.Период
	...
	Если ОтборПоКонтрагенту <> Неопределено Тогда
		Если ОтборПоКонтрагенту.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно
			Или ОтборПоКонтрагенту.ВидСравнения = ВидСравненияКомпоновкиДанных.ВИерархии
			Или ОтборПоКонтрагенту.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСписке
			Или ОтборПоКонтрагенту.ВидСравнения = ВидСравненияКомпоновкиДанных.ВСпискеПоИерархии Тогда
			УсловиеОтбораПоКонтрагенту = "Контрагент В ИЕРАРХИИ(&КонтрагентДляОтбора)";
		ИначеЕсли ОтборПоКонтрагенту.ВидСравнения = ВидСравненияКомпоновкиДанных.НеРавно
			Или ОтборПоКонтрагенту.ВидСравнения = ВидСравненияКомпоновкиДанных.НеВИерархии
			Или ОтборПоКонтрагенту.ВидСравнения = ВидСравненияКомпоновкиДанных.НеВСписке
			Или ОтборПоКонтрагенту.ВидСравнения = ВидСравненияКомпоновкиДанных.НеВСпискеПоИерархии Тогда
			УсловиеОтбораПоКонтрагенту = "НЕ Контрагент В ИЕРАРХИИ(&КонтрагентДляОтбора)";
		КонецЕсли; 
	КонецЕсли;
	Если НЕ ПустаяСтрока(УсловиеОтбораПоКонтрагенту) Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеОтбораПоКонтрагенту", УсловиеОтбораПоКонтрагенту);
		Запрос.УстановитьПараметр("КонтрагентДляОтбора", ОтборПоКонтрагенту.Значение);
	Иначе 
		Запрос.УстановитьПараметр("УсловиеОтбораПоКонтрагенту", Истина);
	КонецЕсли; 
	...
	Пока ВыборкаПоПериодам.Следующий() Цикл
		...
		Пока ВыборкаПоКонтрагентам.Следующий() Цикл
			...
			// расчёт показателей
			Для Каждого Показатель Из МассивПоказателейДляРасчета Цикл
				ВыводитьПредупреждения = (ВыборкаПоПериодам.Период >= Дата1); // предупреждения выводим только при расчёте показателей тех периодов, которые войдут в отчёт
				ТекЗнач = ОбщегоНазначенияСервер.РассчитатьЗначениеПоказателя(ЗначенияПоказателей, Показатель,, ВыводитьПредупреждения);
				Установлен = Истина;
...
Функция ПолучитьСтруктуруОтбораПоКонтрагенту(НастройкиКомпоновки) Экспорт 
	...
	Если КонтрагентДляОтбора = Неопределено Тогда
		Возврат Неопределено;
	КонецЕсли;  
	
	Возврат Новый Структура("Значение, ВидСравнения", КонтрагентДляОтбора, КонтрагентДляОтбора_ВидСравнения); 
